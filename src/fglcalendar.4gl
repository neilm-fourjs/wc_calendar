...
PUBLIC FUNCTION setText(tmpdict)
	DEFINE tmpdict DICTIONARY OF STRING
	CALL textar.clear()
...
#+
PUBLIC FUNCTION getSelectedDateFromValue(l_id, l_value)
	DEFINE l_id SMALLINT, l_value STRING
	DEFINE l_rec RECORD
		 id VARCHAR(50)
	END RECORD,
	l_dte DATE
	...
	IF l_rec.id[1,4] = "sel_" THEN -- Formatted value is "sel_YYYY-MM"
		LET calendars[l_id].view_year = l_rec.id[5,8]
		LET calendars[l_id].view_month = l_rec.id[10,11]
